// front end info: https://www.telerik.com/blogs/build-a-chat-app-with-twilio-and-kendoreact
...
var twiliochat = (function () {
    ...
    function fetchAccessToken(username, handler) {
        $.post('/token', { identity: username }, null, 'json')
            ...
            .fail(function (error) {
                winston.info('Failed to fetch the Access Token with error: ' + error);
            });
    }
    ...
    tc.loadChannelList = function (handler) {
        if (tc.messagingClient === undefined) {
            winston.info('Client is not initialized');
            return;
        }
    ...
    tc.joinGeneralChannel = function () {
        winston.info('Attempting to join "general" chat channel...');
        if (!tc.generalChannel) {
            ...
            tc.messagingClient.createChannel({
                ...
                winston.info('Created general channel');
                tc.generalChannel = channel;
                tc.loadChannelList(tc.joinGeneralChannel);
            });
        }
        else {
            winston.info('Found general channel:');
            setupChannel(tc.generalChannel);
        }
    };

    function initChannel(channel) {
        winston.info('Initialized channel ' + channel.friendlyName);
    ...
    function joinChannel(_channel) {
        return _channel.join()
            .then(function (joinedChannel) {
                winston.info('Joined channel ' + joinedChannel.friendlyName);
    ...
    function initChannelEvents() {
        winston.info(tc.currentChannel.friendlyName + ' ready.');
    ...
    function leaveCurrentChannel() {
        if (tc.currentChannel) {
            return tc.currentChannel.leave().then(function (leftChannel) {
                winston.info('left ' + leftChannel.friendlyName);
    ...
    function deleteCurrentChannel() {
        ...
        tc.currentChannel
            .delete()
            .then(function (channel) {
                winston.info('channel: ' + channel.friendlyName + ' deleted');
                setupChannel(tc.generalChannel);
            });
    }
